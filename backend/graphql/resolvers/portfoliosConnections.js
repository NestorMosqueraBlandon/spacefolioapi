...
const coinbaseClient = new Coinbase.Client({
  accessToken:
    "638adf181444ba8972ce76b77246b3043891be3008ad8f06558207b0dad75acd",
});
...
`;
      ...
      if (data.ethereum.address[0].balances) {
        walletsItems.tokens.push(
          ...data.ethereum.address[0].balances.filter((bal) => bal.value > 0)
        );
...
const walletsRequests = async (address, network) => {
  ...
  switch (network) {
    case "cardano":
      wallets.tokens.push(
        ...data.cardano.address[0].balance.filter((bal) => bal.value > 0)
      );
      break;
    case "bitcoin" ||
      "bitcash" ||
      ...
      "zcash":
      wallets.tokens.push(
        ...data.bitcoin.address[0].balance.filter((bal) => bal.value > 0)
      );
      break;
    case "bsc" || "ethereum" || "velas" || "matic" || "goerlic":
      wallets.tokens.push(
    ...
    case "eos":
      wallets.tokens.push(
        ...data.eos.address[0].balance.filter((bal) => bal.value > 0)
      );
      break;
    case "tron":
      wallets.tokens.push(
        ...data.tron.address[0].balance.filter((bal) => bal.value > 0)
      );
      break;
    case "algoran":
      wallets.tokens.push(
        ...data.algoran.address[0].balance.filter((bal) => bal.value > 0)
      );
      break;
    case "binance":
      wallets.tokens.push(
        ...data.binance.address[0].balance.filter((bal) => bal.value > 0)
      );
      break;
    case "elrond":
      wallets.tokens.push(
        ...data.elrond.address[0].balance.filter((bal) => bal.value > 0)
      );
      break;
    default:
      break;
...
const coingeckoRequests = async (type, cryptos) => {
  ...
  cryptos.tokens.forEach((token) => {
    cryptosCoins.push(
      ...coinList.data.filter(
        (coin) =>
          token.currency.symbol.toLowerCase() == coin.symbol.toLowerCase()
      )
    );
  });
  ...
  cryptos.tokens.forEach((token) => {
    if (type == 0) {
      let arrayResult = Object.assign(
        {
          quantity: token.value,
          apiSymbol: token.currency.symbol.toLowerCase(),
        },
        ...cryptosCoinMarket.filter((coin) =>
          (token.currency.tokenType == "" ||
            (!token.currency.tokenType && token.currency.tokenId == null)) &&
          token.currency.symbol.toLowerCase() == coin.symbol
            ? coin
            : from(Object.values(coin.platforms))
                .where((platform) => platform == token.currency.address)
                .firstOrDefault()
        )
      );
      ...
      let arrayResult = Object.assign(
        {
          quantity: token.currency.quantity
            ? token.currency.quantity
            : token.value,
          apiSymbol: token.currency.symbol.toLowerCase(),
        },
        ...exchangeCoinMarket.filter(
          (coin) => token.currency.symbol.toLowerCase() == coin.symbol
        )
      );
...
const updateCryptos = async (
  ...
  if (
    walletApi.length >=
    userData.portfolios[portfolioIde].wallets[wallet].tokens.length
  ) {
    walletApi.map((walletApiValue) => {
      try {
        from(userData.portfolios[portfolioIde].wallets[wallet].tokens)
          .where(
            (walletDbValue) =>
              walletApiValue.currency.symbol == walletDbValue.symbol
          )
          .first();
      } catch (err) {
    ...
    marketDataCryptos.map((value, index) => {
      walletDatabase.tokens.push({
        coinId: value.coinId,
        quantity: newCryptos.tokens[index].currency.quantity,
        symbol: newCryptos.tokens[index].currency.symbol,
      });
    });
  }
  ...
  if (walletApiNewCryptosRemoved.length > 0) {
    for (let index = 0; index < walletApiNewCryptosRemoved.length; index++) {
      if (
        walletApiNewCryptosRemoved[index].currency.quantity !=
        walletTestDatabase[index].quantity
      ) {
        walletDatabase.tokens[index].quantity =
          walletApiNewCryptosRemoved[index].currency.quantity;
      }
    }
  } else {
    for (let index = 0; index < walletApi.length; index++) {
      if (
        walletApi[index].currency.quantity !=
        walletDatabase.tokens[index].quantity
      ) {
        walletDatabase.tokens[index].quantity =
          walletApi[index].currency.quantity;
...
export default {
  Query: {
    async getWalletsConnection(_, { portfolioId }, context) {
      ...
      try {
        ...
        const portfolioIde = userData.portfolios.findIndex(
    ...
    async getExchangeOrWalletData(
      _,
      { portfolioId, exchangeOrWalletId, type },
      context
    ) {
      const user = checkAuth(context);
      try {
        ...
        if (portfolio) {
          ...
          if (type == 0) {
            ...
            const wallet = userData.portfolios[portfolioIde].wallets.findIndex(
              (wallet) => wallet.id === exchangeOrWalletId
            );
            if (wallet >= 0) {
              const query = `
             query ($network: EthereumNetwork!, $address: String!) {
            ...
            `;
              ...
              if (data && data.ethereum.address[0].balances) {
                wallets.tokens.push(
                  ...data.ethereum.address[0].balances.filter(
                    (bal) => bal.value > 0
                  )
                );
              }
            ...
            wallets.tokens.forEach((token) => {
              walletCoins.push(
                ...coinList.data.filter(
                  (coin) =>
                    token.currency.symbol.toLowerCase() ==
            ...
            for (let i = 0; i < walletCoins.length; i++) {
              const { data } = await CoinGeckoClient.coins.fetch(
                walletCoins[i].id
              );
              const {
                id: coinId,
            ...
            metadata.tokens = from(portfolioTokens)
              .groupBy(
                ...
                (key, t) => {
                  return {
                    ...
                    value_usd_24h:
                      ((t.sum((token) => token["quantity"]) * t.first().usd) /
                        100) *
                      t.first().price_change_percentage_24h,
                    price_change_percentage_24h:
                      (100 /
                        (t.sum((token) => token["quantity"]) * t.first().usd)) *
                      (((t.sum((token) => token["quantity"]) * t.first().usd) /
            ...
            metadata.tokens = metadata.tokens.filter(
              (crypto) => crypto.symbol != undefined
            );
            metadata.balance = metadata.tokens.reduce(
              (a, c) => a + c.value * 1,
              0
            );
          ...
          if (type == 1) {
            ...
            const exchange = userData.portfolios[
              portfolioIde
            ].exchanges.findIndex(
              (exchange) => exchange.id === exchangeOrWalletId
            );
            if (exchange >= 0) {
              const coinList = await CoinGeckoClient.coins.list();
              ...
              if (portfolio.exchanges[exchange].network === "binance") {
                ...
                portfolioTokens.forEach((token) => {
                  ...
                  newExchangesTokens.sort(
                    (a, c) => a.coingecko_rank - c.coingecko_rank
                  );
                });
                ...
                metadata.tokens = from(newExchangesTokens)
                  .groupBy(
                    (tokens) => tokens.symbol,
                    null,
                    (key, t) => {
                      return {
                        ...
                        value_usd_24h:
                          ((t.sum((token) => token["quantity"]) *
                            t.first().usd) /
                            100) *
                          t.first().price_change_percentage_24h,

                        value_usd_7d:
                          (t.first().price_change_percentage_7d *
                            (t.sum((token) => token["quantity"]) *
                              t.first().usd)) /
                            100 ==
                          0
                        ...
                        value_usd_30d:
                          (t.first().price_change_percentage_30d *
                            (t.sum((token) => token["quantity"]) *
                              t.first().usd)) /
                            100 ==
                          0
                        ...
                        value_usd_1y:
                          (t.first().price_change_percentage_1y *
                            (t.sum((token) => token["quantity"]) *
                              t.first().usd)) /
                            100 ==
                          0
      ...
      try {
        ...
        for (let i = 0; i < portfolios.length; i++) {
          ...
          if (portfolios[i].wallets.length > 0 && !getInternalData) {
            for (let j = 0; j < portfolios[i].wallets.length; j++) {
              if (portfolios[i].wallets[j]) {
                // console.log(portfolios[i].wallets[j].network)
            ...
            for (let j = 0; j < portfolios[i].wallets.length; j++) {
              ...
              if (portfolios[i].wallets[j].network == "cardano") {
                if (data && data.cardano.address[0].balance) {
                  wallets.tokens.push(
                    ...data.cardano.address[0].balance.filter(
                      (bal) => bal.value > 0
                    )
                  );
                }
        ...
        if (!getInternalData) {
          ...
          metadata.cryptos = from(portfolioTokens)
            .groupBy(
              (tokens) => tokens.symbol,
              null,
              (key, t) => {
          ...
          for (let i = 0; i < portfolios.length; i++) {
            metadata.cryptos = metadata.cryptos.filter(
              (crypto) => crypto.symbol != undefined
            );
            metadata.balance = metadata.cryptos.reduce(
              (a, c) => a + c.value * 1,
              0
            );

            let sumPercentage = metadata.cryptos.reduce(
              (a, c) => a + c.price_change_percentage_30d * 1,
              0
            );
            let sumPercentageUsd = metadata.cryptos.reduce(
              (a, c) => a + c.value_usd_7d * 1,
              0
            );

            let avg =
              metadata.cryptos.length > 0
                ? sumPercentage / metadata.cryptos.length
                : 0;
            let avgUsd =
              metadata.cryptos.length > 0
                ? sumPercentageUsd / metadata.cryptos.length
                : 0;
            ...
            firtsArray.push({
              ...
              price_change_percentage: avg,
              value_usd: avgUsd,
            });
            // arrayPortfolios.push({ name: portfolios[i].name, balance: metadata.balance })

            for (let k = 0; k < portfolios.length; k++) {
              if (firtsArray[k] != undefined) {
                console.log(firtsArray[k].balance);
                let percentage = (firtsArray[k].balance / totalBalance) * 100;
                arrayPortfolios.push({
                  id: firtsArray[k].id,
                  name: firtsArray[k].name,
                  balance: firtsArray[k].balance,
                  price_change_percentage:
                    firtsArray[k].price_change_percentage,
                  percentage,
                  value_usd: firtsArray[k].value_usd,
                });
              }
            }
          }
    ...
    async getMetadataPortfolio(
      _,
      { portfolioId, userId, interval = false },
      context
    ) {
      const user = checkAuth(context);
        ...
        for (let j = 0; j < portfolio.wallets.length; j++) {
          for (let i = 0; i < portfolio.wallets[j].tokens.length; i++) {
            if (portfolio.wallets[j].tokens[i].coinId) {
              const { data } = await CoinGeckoClient.coins.fetch(
                portfolio.wallets[j].tokens[i].coinId
              );
              const {
                id: coinId,
                symbol,
            ...
            portfolio.wallets[j].tokens.forEach((token) => {
              ...
              let arrayResult = Object.assign(
                ...
                ...walletCoinMarket.filter(
                  (coin) => token.coinId == coin.coinId
                )
              );
          ...
          if (portfolio.exchanges[j].network === "binance") {
            ...
            exchangePortfolioTokens.forEach((token) => {
              let arrayResult = Object.assign(
                ...
                ...exchangeCoinMarket.filter(
                  (coin) =>
                    token.currency.name.toLowerCase() == coin.name.toLowerCase()
                )
              );
              newExchangesTokens.sort(
                (a, c) => a.coingecko_rank - c.coingecko_rank
        ...
        .groupBy(
          ...
          (key, t) => {
            return {
              ...
              value_usd_24h:
                ((t.sum((token) => token["quantity"]) * t.first().usd) / 100) *
                t.first().price_change_percentage_24h,
              ...
              price_change_percentage_24h:
                (100 / (t.sum((token) => token["quantity"]) * t.first().usd)) *
                (((t.sum((token) => token["quantity"]) * t.first().usd) / 100) *
                  t.first().price_change_percentage_24h),
      ...
      metadata.price_change_percentage_24h = metadata.cryptos.reduce(
        (a, c) => a + c.price_change_percentage_24h * 1,
        0
      );
      metadata.price_change_percentage_7d = metadata.cryptos.reduce(
        (a, c) => a + c.price_change_percentage_7d * 1,
        0
      );
      metadata.price_change_percentage_30d = metadata.cryptos.reduce(
        (a, c) => a + c.price_change_percentage_30d * 1,
        0
      );
      metadata.price_change_percentage_1y = metadata.cryptos.reduce(
        (a, c) => a + c.price_change_percentage_1y * 1,
        0
      );
      metadata.value_24h = metadata.cryptos.reduce(
        (a, c) => a + c.value_usd_24h * 1,
        0
      );
      metadata.value_7d = metadata.cryptos.reduce(
        (a, c) => a + c.value_usd_7d * 1,
        0
      );
      metadata.value_30d = metadata.cryptos.reduce(
        (a, c) => a + c.value_usd_30d * 1,
        0
      );
      metadata.value_1y = metadata.cryptos.reduce(
        (a, c) => a + c.value_usd_1y * 1,
        0
      );
      ...
      if (interval) {
        chart = JSON.stringify(
          await portfolioChart(
            metadata.balance,
            JSON.parse(userData.portfolioGeneralChart)
          )
        );
  ...
  Mutation: {
    ...
    async updateDataConection(
      _,
      { portfolioId, walletId, exchangeId },
      context
    ) {
      ...
      if (walletId == "" && exchangeId == "") {
        try {
          ...
          if (portfolio) {
            ...
            for (
              let l = 0;
              l < userData.portfolios[portfolioIde].wallets.length;
              l++
            ) {
              const wallet = l;
              const walletApi = await walletsRequests(
                userData.portfolios[portfolioIde].wallets[wallet].address,
                userData.portfolios[portfolioIde].wallets[wallet].network
              );
              ...
              for (let index = 0; index < walletApi.tokens.length; index++) {
                for (
                  let j = 0;
                  j <
                  userData.portfolios[portfolioIde].wallets[wallet].tokens
                    .length;
                  j++
                ) {
                  if (
                    walletApi.tokens[index].currency.symbol.toLowerCase() ==
                    userData.portfolios[portfolioIde].wallets[wallet].tokens[j]
                      .apiSymbol
                  ) {
                    equals++;
                  }
              ...
              if (equals != walletApi.tokens.length - 1) {
                ...
                for (
                  ...
                  for (var j = 0; j < walletApi.tokens.length; j++) {
                    if (
                      ...
                      walletApi.tokens[j].currency.symbol.toLowerCase()
                    ) {
                      walletRestore.push(
                ...
                userData.portfolios[portfolioIde].wallets[
                  wallet
                ].tokens = walletRestore;
                ...
                walletApi.tokens.map((walletApiValue) => {
                  try {
                    from(
                      ...
                      .where(
                        (walletDbValue) =>
                          walletApiValue.currency.symbol.toLowerCase() ==
                          walletDbValue.apiSymbol
                      )
                      .first();
                  } catch (err) {
                    newCryptos.tokens.push(walletApiValue);
                ...
                marketDataCryptos.map((value, index) => {
                  userData.portfolios[portfolioIde].wallets[
                    ...
                    coinId: value.coinId,
                    quantity: newCryptos.tokens[index].value,
                    symbol: newCryptos.tokens[index].currency.symbol,
                  });
                });
              }
              ...
              for (let index = 0; index < walletApi.tokens.length; index++) {
                for (
                  ...
                  if (
                    ...
                    userData.portfolios[portfolioIde].wallets[wallet].tokens[
                      j
                    ].quantity = walletApi.tokens[index].value;
                  }
                }
              }
            }
            ...
            for (
              let l = 0;
              l < userData.portfolios[portfolioIde].exchanges.length;
              l++
            ) {
              const wallet = l;
              const walletApi = await walletsRequests(
                userData.portfolios[portfolioIde].exchanges[wallet].address,
                userData.portfolios[portfolioIde].exchanges[wallet].network
              );
              ...
              for (let index = 0; index < walletApi.tokens.length; index++) {
                for (
                  let j = 0;
                  j <
                  userData.portfolios[portfolioIde].exchanges[wallet].tokens
                    .length;
                  j++
                ) {
                  if (
              ...
              if (equals != walletApi.tokens.length - 1) {
                ...
                for (
                  ...
                  for (var j = 0; j < walletApi.tokens.length; j++) {
                    if (
                      userData.portfolios[portfolioIde].exchanges[wallet]
                        .tokens[i].apiSymbol ===
                      walletApi.tokens[j].currency.symbol.toLowerCase()
                    ) {
                      walletRestore.push(
                ...
                userData.portfolios[portfolioIde].exchanges[
                  wallet
                ].tokens = walletRestore;
          ...
          if (portfolio) {
            ...
            if (equals != walletApi.tokens.length - 1) {
              ...
              for (
                ...
                for (var j = 0; j < walletApi.tokens.length; j++) {
                  if (
                    userData.portfolios[portfolioIde].wallets[wallet].tokens[i]
                      .apiSymbol ===
                    walletApi.tokens[j].currency.symbol.toLowerCase()
                  ) {
                    walletRestore.push(
              ...
              let marketDataCryptos = await coingeckoRequests(0, newCryptos);
              marketDataCryptos.map((value, index) => {
                userData.portfolios[portfolioIde].wallets[wallet].tokens.push({
                  coinId: value.coinId,
                  quantity: newCryptos.tokens[index].value,
                  symbol: newCryptos.tokens[index].currency.symbol,
                });
        ...
        try {
          ...
          const portfolio = userData.portfolios[portfolioIde];
          if (portfolio) {
            const wallet = userData.portfolios[
              portfolioIde
            ].exchanges.findIndex((wallet) => wallet.id === exchangeId);
            ...
            if (
              ...
              const client = new MainClient({
                api_key:
                  userData.portfolios[portfolioIde].exchanges[wallet].apiKey,
                api_secret:
                  userData.portfolios[portfolioIde].exchanges[wallet].apiSecret,
              });

              const data = await client.getBalances();

              const tokens = data.filter((token) => token.free > 0);
              const tokensQuantity = tokens.reduce(
                (a, c) => a + Number(c.free),
                0
              );
            ...
            for (let index = 0; index < walletApi.tokens.length; index++) {
              for (
                ...
                if (
                  walletApi.tokens[index].currency.symbol.toLowerCase() ==
                  userData.portfolios[portfolioIde].exchanges[wallet].tokens[j]
                    .symbol
                ) {
                  equals++;
                }
            ...
            if (equals != walletApi.tokens.length - 1) {
              ...
              let marketDataCryptos = await coingeckoRequests(0, newCryptos);
              marketDataCryptos.map((value, index) => {
                userData.portfolios[portfolioIde].exchanges[wallet].tokens.push(
                  newCryptos.tokens[index]
                );
              });
            }
            ...
            for (let index = 0; index < walletApi.tokens.length; index++) {
              ...
              for (
                ...
                if (
                  ...
                  userData.portfolios[portfolioIde].exchanges[wallet].tokens[
                    j
                  ].value = walletApi.tokens[index].value;
                }
              }
            }
    ...
    async updateWalletConnection(
      _,
      { name, portfolioId, walletId, active, all },
      context
    ) {
      const user = checkAuth(context);
    ...
    async updateExchangeConnection(
      ...
      try {
        ...
        if (portfolio) {
          const exchange = userData.portfolios[
            portfolioIde
          ].exchanges.findIndex((exchange) => exchange.id === exchangeId);

          const address =
            userData.portfolios[portfolioIde].exchange[wallet].address;
          const apiKey =
            userData.portfolios[portfolioIde].exchanges[wallet].network;
    ...
    async deleteWalletConnection(_, { portfolioId, walletId }, context) {
      const user = checkAuth(context);
      ...
      try {
        ...
        console.log(portfolio);
        if (portfolio) {
          const walletIndex = userData.portfolios[
            portfolioIde
          ].wallets.findIndex((w) => w.id === walletId);
    ...
    async deleteExchangeConnection(_, { portfolioId, exchangeId }, context) {
      const user = checkAuth(context);

      try {
        const portfolio = await Portfolio.findById(portfolioId);
        if (portfolio) {
          const exchangeIndex = portfolio.exchanges.findIndex(
            (w) => w.id === exchangeId
          );
    ...
    async addExchangeConnection(
      ...
      if (userData.portfolios[portfolioIde].exchanges.length > 0) {
        userData.portfolios[portfolioIde].exchanges.map((exchange) => {
          if (
            exchange.name == name ||
            (exchange.apiKey == key && exchange.network == network)
          ) {
            throw new Error(801);
          }
        });
